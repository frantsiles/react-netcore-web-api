...
| -------- | ------------------------------------------- | -------------------------------- |
| **E0**   | Tenant-aware design                         | ✅ Completa (87 tests)           |
| **E1**   | Parties + Catalog                           | ✅ Completa (68 tests)           |
| **E2**   | Sales pipeline                              | 🟡 Backend + frontend ✅, profundizar |
| **E3**   | Inventory (event sourcing)                  | 🟡 Backend + frontend ✅, profundizar |
| **E3.5** | Switch a control plane real + BD-por-tenant | ⬜ Pendiente                     |
| **E4**   | Purchasing                                  | 🟡 Backend + frontend ✅, profundizar |
| **E5**   | Accounting (event sourcing)                 | 🟡 Backend + frontend ✅, profundizar |
| **E6**   | Banking + Reconciliations                   | 🟡 Backend + frontend ✅, profundizar |
| **E7**   | Tax engine completo + Approvals             | 🟡 Backend + frontend ✅, profundizar |
| **E8**   | Reporting consolidado multi-tenant          | 🟡 Backend + frontend ✅, profundizar |
| **E9**   | Personal — HR limitado                      | 🟡 Backend + frontend ✅, profundizar |
...
- `Configurar Postgres + migraciones EF` — ✅ completado (sesión 2026-05-17)
...
## Actualización de sesión (2026-05-17) — PostgreSQL + Migraciones EF Core

Se completó la configuración de Postgres real para todos los módulos ERP:

- **`HasDefaultSchema()`** añadido a los 11 DbContexts de módulo (`parties`, `catalog`, `sales`, `purchasing`, `inventory`, `accounting`, `banking`, `tax`, `approvals`, `hr`, `controlplane`).
- **`IDesignTimeDbContextFactory<T>`** creado por módulo para que `dotnet ef` funcione sin startup project.
- **`DesignTimeTenantContext`** (stub de `ITenantContext`) añadido en `Api.Infrastructure.Persistence`.
- **Paquete `Microsoft.EntityFrameworkCore.Design`** agregado a los 11 proyectos Infrastructure.
- **Migración `InitialCreate`** generada para los 11 módulos (30 tablas en 11 esquemas).
- **`ModuleMigrator`** creado en `Api.WebApi` — aplica las migraciones de todos los módulos al arrancar.
- **`Program.cs`** actualizado: llama `ModuleMigrator.MigrateAllAsync()` tras `DatabaseInitializer.InitializeAsync()`.
- **Tax, Approvals, HR** DI corregido: estaban hardcodeados a InMemory; ahora siguen el patrón `connection string → Npgsql, vacío → InMemory`.
- **Purchasing y Accounting DbContext** corregidos: `OwnsMany<T>("_field", ...)` → `OwnsMany(e => e.Property, ...)` para compatibilidad con EF Core design-time.

**Resultado:** `demodb` en Postgres con 12 esquemas y 30 tablas. La API aplica todas las migraciones al arrancar en entorno Development.
