...
test.describe("Inventory flow", () => {
  test("admin can create a warehouse and switch tabs without freezing", async ({
    page,
  }) => {
    ...
    await expect(inventoryPage.itemsTab).toHaveAttribute(
      "data-state",
      "active",
    );
    await expect(inventoryPage.warehousesTab).toHaveAttribute(
      "data-state",
      "inactive",
    );
