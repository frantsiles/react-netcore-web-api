...
import {
  Plus, MoreHorizontal, Warehouse,
  PackageSearch, ChevronUp, ChevronDown, AlertTriangle,
} from 'lucide-react'
...
export function InventoryPage() {
  ...
  const { data: items, isLoading: itemsLoading } = useQuery({
    ...
    queryFn: () => inventoryService.items.search({
  ...
  const receiveMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['inventory'] }); setIsReceiveOpen(false); toast.success('Stock recibido') },
  ...
  const adjustMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['inventory'] }); setIsAdjustOpen(false); toast.success('Ajuste registrado') },
    onError:   () => toast.error('Error al ajustar stock'),
  })

  const createWHMut = useMutation({
    mutationFn: inventoryService.warehouses.create,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['warehouses'] }); setIsWHOpen(false); toast.success('Almacén creado') },
    onError:   () => toast.error('Error al crear almacén'),
  })

  // ── Forms ──────────────────────────────────────────────────────────────────

  const receiveForm  = useForm<ReceiveForm>({  resolver: zodResolver(receiveSchema),  defaultValues: { quantity: 1 } })
  ...
  const itemColumns = [
    ...
    iCol.accessor('quantityAvailable', {
      header: 'Disponible',
      cell: i => <span className="font-medium text-green-700 dark:text-green-400">{i.getValue()}</span>,
    }),
    ...
    iCol.accessor('isLowStock', {
      header: 'Alerta',
      cell: i => i.getValue()
        ? <Badge variant="destructive" className="gap-1"><AlertTriangle className="h-3 w-3" />Bajo stock</Badge>
        : null,
    }),
    iCol.display({
      id: 'actions',
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => openAdjustDialog(row.original.id)}>Ajustar</DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem className="text-destructive" onClick={() => {
              const reason = prompt('Motivo del castigo:')
              if (reason) inventoryService.items.writeOff(row.original.id, { quantity: 1, reason })
                .then(() => { qc.invalidateQueries({ queryKey: ['inventory'] }); toast.success('Castigo registrado') })
                .catch(() => toast.error('Error al registrar castigo'))
            }}>Castigo/Merma</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ),
    }),
  ...
  const warehouseColumns = [
    ...
    wCol.accessor('status', {
      header: 'Estado',
      cell: i => <Badge variant={i.getValue() === 'Active' ? 'success' : 'secondary'}>
  ...
  const itemTable = useReactTable({
    ...
    state: { sorting: iSorting }, onSortingChange: setISorting,
    getCoreRowModel: getCoreRowModel(), getSortedRowModel: getSortedRowModel(),
  })

  const whTable = useReactTable({
    data: warehouses ?? [], columns: warehouseColumns,
    state: { sorting: wSorting }, onSortingChange: setWSorting,
    getCoreRowModel: getCoreRowModel(), getSortedRowModel: getSortedRowModel(),
  })
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Existencias por almacén</p>
        </div>
        {activeTab === 'items' ? (
          <Button onClick={() => { receiveForm.reset({ quantity: 1 }); setIsReceiveOpen(true) }}>
            <Plus className="mr-2 h-4 w-4" />Recibir stock
          </Button>
        ) : (
          <Button onClick={() => { whForm.reset(); setIsWHOpen(true) }}>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList>
          <TabsTrigger value="items">
            <PackageSearch className="mr-2 h-4 w-4" />Existencias
          </TabsTrigger>
        ...
        <TabsContent value="items" className="space-y-4">
          ...
          <SimpleTable table={itemTable} columns={itemColumns} isLoading={itemsLoading}
            emptyMessage="No hay existencias registradas" />
        </TabsContent>

        <TabsContent value="warehouses" className="space-y-4">
          <SimpleTable table={whTable} columns={warehouseColumns} isLoading={whLoading}
      ...
      <Dialog open={isReceiveOpen} onOpenChange={setIsReceiveOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader><DialogTitle>Recibir Stock</DialogTitle></DialogHeader>
          <form onSubmit={receiveForm.handleSubmit(v => receiveMut.mutate({
            ...v,
            ...
            <div className="space-y-1.5">
              ...
              <CatalogItemPicker
                ...
                onChange={(id, sku) => {
                  receiveForm.setValue('catalogItemId', id, { shouldValidate: true })
                  receiveForm.setValue('sku', sku, { shouldValidate: true })
                }}
              />
              {receiveForm.formState.errors.catalogItemId && <p className="text-xs text-destructive">{receiveForm.formState.errors.catalogItemId.message}</p>}
            </div>
            <div className="space-y-1.5">
              <Label>Almacén *</Label>
              <WarehousePicker
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label>SKU *</Label>
                <Input {...receiveForm.register('sku')} placeholder="PROD-001" readOnly className="bg-muted" />
              </div>
              <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input {...receiveForm.register('referenceNumber')} placeholder="OC-12345" />
              </div>
              <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              <Label>Notas</Label>
              <Input {...receiveForm.register('notes')} placeholder="Opcional" />
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setIsReceiveOpen(false)}>Cancelar</Button>
              <Button type="submit" disabled={receiveMut.isPending}>{receiveMut.isPending ? 'Guardando…' : 'Recibir'}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isAdjustOpen} onOpenChange={setIsAdjustOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <form onSubmit={adjustForm.handleSubmit(v => adjustMut.mutate({ id: selectedItemId!, body: v }))} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label>Motivo *</Label>
              <Input {...adjustForm.register('reason')} placeholder="Describe el motivo del ajuste" />
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={adjustMut.isPending}>{adjustMut.isPending ? 'Guardando…' : 'Ajustar'}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isWHOpen} onOpenChange={setIsWHOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <form onSubmit={whForm.handleSubmit(v => createWHMut.mutate({ ...v, address: v.address || undefined }))} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label>Código *</Label>
              ...
              <div className="space-y-1.5">
                <Label>Nombre *</Label>
                <Input {...whForm.register('name')} placeholder="Almacén Ciudad de México" />
              </div>
            </div>
            <div className="space-y-1.5">
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setIsWHOpen(false)}>Cancelar</Button>
              <Button type="submit" disabled={createWHMut.isPending}>{createWHMut.isPending ? 'Creando…' : 'Crear'}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
...
function SimpleTable<T>({
  table, columns, isLoading, emptyMessage,
}: {
      ...
      <Table>
        ...
        <TableBody>
          {isLoading ? (
            ...
            <TableRow>
              <TableCell colSpan={columns.length as number} className="h-24 text-center text-muted-foreground">
                {emptyMessage}
              </TableCell>
            </TableRow>
          ) : (
