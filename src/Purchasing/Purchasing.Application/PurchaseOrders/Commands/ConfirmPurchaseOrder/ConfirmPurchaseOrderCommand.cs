...
namespace Purchasing.Application.PurchaseOrders.Commands.ConfirmPurchaseOrder;

public record ConfirmPurchaseOrderCommand(Guid PurchaseOrderId) : IRequest<PurchaseOrderDto>;
...
public class ConfirmPurchaseOrderCommandHandler : IRequestHandler<ConfirmPurchaseOrderCommand, PurchaseOrderDto>
{
    ...
    public ConfirmPurchaseOrderCommandHandler(IPurchaseOrderRepository repo, IEventPublisher events)
        => (_repo, _events) = (repo, events);

    public async Task<PurchaseOrderDto> Handle(ConfirmPurchaseOrderCommand cmd, CancellationToken ct)
    {
